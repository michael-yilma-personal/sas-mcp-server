...
| `MCP_CORS_ORIGINS` | No | — (empty: no CORS policy) | HTTP mode: comma-separated browser origins allowed to call the MCP endpoint cross-origin, e.g. `https://app.example.com`. A browser-based MCP client on another origin needs this for its preflight to pass and to read `mcp-session-id` off the response; MCP clients that are not browsers are unaffected. Authentication is unchanged — only the origin list widens. |
