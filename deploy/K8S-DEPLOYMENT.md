...
| `MCP_TIERS` / `MCP_READ_ONLY` | `0-8` / `false` | Worth setting deliberately for a shared deployment rather than exposing all 75 tools. |
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: server
          image: ghcr.io/sassoftware/sas-mcp-server:1.11.2
